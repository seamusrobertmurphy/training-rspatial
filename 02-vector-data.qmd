...
Now the area reconciliation. A claimed area is compared with the geometry's measured area; the auditor reports the difference. Measure it two ways, once from the bounding box and once from the convex hull of the stems:

```{r}
claimed_ha <- 25.6                                    # nominal 400 x 640 m plot
bbox_ha <- as.numeric(st_area(st_as_sfc(st_bbox(stems)))) / 1e4
hull_ha <- as.numeric(st_area(st_convex_hull(st_union(stems)))) / 1e4
...
The two disagree, and the disagreement is the lesson. The plot grid runs about 1.8 degrees off UTM north, and a bounding box is axis-aligned by construction, so it wraps a tilted rectangle inside a larger upright one. The convex hull follows the stems and recovers the nominal area almost exactly. Neither figure reflects a survey error. When a claimed area and a mapped area disagree, the first question is not whether the survey is wrong but whether the thing being measured is the boundary or a box drawn around it, because that choice alone moved the answer by more than a hectare.
...
```{r}
e  <- st_bbox(stems)                                  # two parcels on the plot
