---
title: "Selecting an allometric equation and how model choice propagates into a reported carbon stock"
aliases:
  - allometry.html
execute:
  eval: true
...
Fitting on that scale is standard practice, but it introduces a trap. Exponentiating a prediction from log space returns the median rather than the mean of the distribution, which systematically underestimates biomass. The correction is a multiplicative factor $\exp(\hat{\sigma}^2/2)$ [@baskerville1972], and it is used throughout this book. The uncertainty chapter shows what it does to the error statistics, and the [statistical primers appendix](A1-primers.qmd) shows the skew that makes the correction necessary.
...
The median published equation is calibrated over a range that contains about 61 per cent of the stems it would be applied to, and the weakest of them covers five. That is already poor, and the next calculation shows it is worse than it looks, because stems are not what matters. Weight each stem by its approximate biomass and ask what share of the *carbon* sits inside the calibration range:
...
The median falls from 61 per cent of stems to 44 per cent of biomass, the weakest quarter of the equations cover under 14 per cent of it, and the worst covers none at all. The reason is structural rather than accidental. Destructive sampling is dominated by small trees because large ones are expensive to fell and rare in the sample, so equations are best supported exactly where the carbon is not. Almost every biomass estimate ever made is an extrapolation over the trees that carry most of its mass.
...
One equation out of twenty-three remains valid at the largest maple in the plot, a stem of 70 centimetres. A project that reports a single point estimate from a single equation is concealing that.
...
The carbon fraction of 0.47 is itself a default with its own uncertainty, and it multiplies everything above it. The uncertainty chapter propagates that.

::: {.verification}
#### Registry Requirements {.unnumbered .unlisted}
...
out at the end of the uncertainty chapter. Under ART-TREES the allometric contribution feeds
