---
title: "Quantifying uncertainty and its cost: Monte Carlo propagation, registry deductions and the limits of a short record"
aliases:
  - 07-uncertainty.html
execute:
  eval: true
...
::: {.verification}
#### Metric to Deduction {.unnumbered .unlisted}
...
::: {.verification}
#### Model Eligibility {.unnumbered .unlisted}
...
Sampling error is only part of the story; the allometric model and the carbon fraction are uncertain too, and their errors compound. When error propagates through a non-linear function like $a\,D^{b}$, the clean way to combine sources is Monte Carlo simulation: draw the uncertain inputs from their distributions many times, recompute the whole estimate each time, and read the spread of results [@ipcc2006gl]. The [statistical primers appendix](A1-primers.qmd) sets out why the algebraic alternative, combining relative errors in quadrature, is the special case rather than the general rule, and how many iterations the simulation needs.
...
::: {.verification}
#### Rule Sources {.unnumbered .unlisted}
...
::: {.verification}
#### Sampling and Counting {.unnumbered .unlisted}
