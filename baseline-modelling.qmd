...
# a risk surface: pressure decays with distance from the western edge
d_edge <- xy[, 1] - min(xy[, 1])
risk   <- exp(-d_edge / 150)
risk_r <- rast(r); values(risk_r) <- risk

zone_rate <- 40                                    # ha/yr across the whole area
alloc     <- risk / sum(risk) * zone_rate
alloc_r   <- rast(r); values(alloc_r) <- alloc

plot(c(risk_r, alloc_r), main = c("Modelled risk", "Allocated loss (ha/yr)"))
...
::: {.verification}
#### Reference periods and revalidation {.unnumbered .unlisted}
...
::: {.verification}
#### What a verifier asks about a baseline {.unnumbered .unlisted}
